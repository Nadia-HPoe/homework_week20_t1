...
function makeThree() {
  ...
  try {
    ...
    resultThree.textContent = "Оператор throw new Error выполнен";
    console.error("Оператор throw new Error выполнен");
  }
}
...
function makeFive() {
  try {
    let inputData = -1;
    if (inputData < 0) throw new Error("Ошибка!");
...
function makeSix() {
  try {
    let inputData = -1;
    if (inputData < 0) throw new Error("Ошибка перехвачена");
  } catch (error) {
    console.log(error.message);
  }
}
...
function makeFourteen() {
  try {
    ...
    if (discount < 0 || discount > 50) {
      ...
      console.log(`Ваша скидка составит: ${discount}%`);
    }
  } catch (error) {
    console.log(error.message);
  }
...
function makeTwenty() {
  try {
    document.getElementById("result20").textContent = calculateValue();
  } catch (error) {
...
function makeTwentyOne() {
  try {
    document.getElementById("result21").textContent = fetchData();
  } catch (error) {
...
function makeTwentyThree() {
  ...
  try {
    let user = JSON.parse(jsonTwentyThree); //Возникает ошибка
    console.log(user.name); //Не сработает
  } catch (e) {
